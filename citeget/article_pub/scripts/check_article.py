        ...
        checks.append(
            {
                "item": "Author bio",
                "present": bool(
                    re.search(r"(?i)(about the author|author bio|biography)", text)
                ),
        ...
        checks.append(
            {
                "item": "Abstract",
                "present": bool(
                    re.search(r"(?i)^#{1,3}\s*abstract", text, re.MULTILINE)
                ),
                "note": "Required",
            }
        )
        ...
        checks.append(
            {
                ...
                "present": bool(
                    re.search(
                        r"(?i)(objective|methods|results|conclusion)", text[:2000]
                    )
                ),
        ...
        checks.append(
            {
                ...
                "present": bool(
                    re.search(r"(?i)^#{1,3}\s*related\s+work", text, re.MULTILINE)
                ),
                "note": "Expected",
            }
        )
        checks.append(
            {
                ...
                "present": bool(
                    re.search(r"(?i)(data\s+availability|replication\s+package)", text)
                ),
...
def run_check(article_path: Path, journal_key: str | None) -> None:
    ...
    if journal_key == "ieee_software":
        ...
        print(
            f"  Word count (+ {fig_table_count} figs×250): {effective_word_count} / {limit} {status}"
        )
    else:
    ...
    print(
        format_count_check(
            abstract_word_count, profile.get("abstract_word_limit"), "Abstract words"
        )
    )
    ...
    if profile.get("page_limit"):
        print(
            f"  Page limit: {profile['page_limit']} {profile.get('page_limit_format', '')} (manual check required)"
        )
    ...
    if profile.get("submission_model") == "invitation_only":
        ...
        print(
            "   You must receive an editorial invitation or pitch to queue-info@acm.org"
        )
    ...
    if profile.get("pre_submission_recommended"):
        print(
            f"  Pre-submission email recommended: {profile.get('pre_submission_contact', 'see guidelines')}"
        )
